...
- (void)setCommentArray {
    ...
    [list addObject:[NSNumber numberWithFloat:[self getRowHeightWithReview:comment1.comment]]];
    ...
    [list addObject:[NSNumber numberWithFloat:[self getRowHeightWithReview:comment2.comment]]];

    MovieComment *comment3 = [MovieComment new];
    comment3.name = @" yueer";
    comment3.comment = @"刚看完由陈哲艺监製，并跨国连三位新生代忻钰坤、陈世杰、Sivaroj Kongsakul导演，陈柏霖、秦沛、蒋雯丽、杨祐宁、小茶领衔主演的《再见，在也不见》，感受是闷、很闷、极度闷。 主角失落的人生让我极度失望。 三个故事，三种情感，三段人生。温吞到沉闷的矫情作品，无法相认的父子俩，万千阻隔的断背情，暗涌深藏的师生恋，每段都矫情， 陈柏霖的表演在我看得好累，亲情、同...........金马影展的开幕影片，一部真正意义上的分段式电影，讲述亲情、友情、爱情三段不同的故事，由三个不同的导演打造，却由同一个人演绎，保持统一的情感基调，别有一番韵味。 《再见，在也不见》，一个很有意思的片名，既然再见，那难道不是再也不见，为何又在，也不见呢，主题曲《在，也不见》更是让人产生无限遐想，而影片的英文名《Distance》则像是在说，即使相见也有那永远无............";
    [list addObject:[NSNumber numberWithFloat:[self getRowHeightWithReview:comment3.comment]]];
    ...
    [list addObject:[NSNumber numberWithFloat:[self getRowHeightWithReview:comment4.comment]]];

    MovieComment *comment5 = [MovieComment new];
    comment5.name = @"水漾薄荷";
    comment5.comment = @"被电影名吸引而进入影院，开始看到这个片名的时候略感意外，为什么是“再见，在也不见”，而不是“再见，再也不见”呢？看了片子后明白，有些事，有些人，错过了，即使还在，见还不如不见，这点我非常认同。有些过去的往事，随风而逝，随时间的流逝，各自已不再是当时的那个自己，即便再见，也不再是当年的模样和心境，何不如留住曾经的美好！存在记忆的心底，作为一种怀念，......金马影展的开幕影片，一部真正意义上的分段式电影，讲述亲情、友情、爱情三段不同的故事，由三个不同的导演打造，却由同一个人演绎，保持统一的情感基调，别有一番韵味。 《再见，在也不见》，一个很有意思的片名，既然再见，那难道不是再也不见，为何又在，也不见呢，主题曲《在，也不见》更是让人产生无限遐想，而影片的英文名《Distance》则像是在说，即使相见也有那永远无...........";
    [list addObject:[NSNumber numberWithFloat:[self getRowHeightWithReview:comment5.comment]]];
    ...
    [commentList addObject:comment1];
    [commentList addObject:comment2];
    [commentList addObject:comment3];
    [commentList addObject:comment4];
    [commentList addObject:comment5];
...
- (void)configureCommentView {
    ...
    NSLayoutConstraint *heightConstraint = [NSLayoutConstraint constraintWithItem:self.movieReview attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:0 multiplier:1 constant:(_commentTableHeight+20)];
    ...
    */
    ...
    [self.commentTable mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.equalTo([NSNumber numberWithFloat:_commentTableHeight]);
    }];
    ...
    CGPathMoveToPoint(paths, NULL, 15, _commentTableHeight+20);
    CGPathAddLineToPoint(paths, NULL, MAIN_WIDTH-15, _commentTableHeight+20);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    */
    CGFloat commentHeight = [[self.rowHeightList objectAtIndex:indexPath.row] floatValue];
    
    return commentHeight + 40;
}
